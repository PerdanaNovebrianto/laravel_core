<?php

return [
    'file_type' => 'Valid file formats are: :values.',
    'file_size' => 'File size must not exceed :max MB.',
    'format_email' => 'Email format is invalid.',
    'format_phone' => 'Phone format is invalid.',
    'max_length' => ':attribute length must not exceed :max characters.',
    'min_length' => ':attribute length must not be less than :min characters.',
    'only_letters' => 'Must only contain letters.',
    'only_numbers' => 'Must only contain numbers.',
    'required' => ':attribute is required.',
    'required_fill' => 'Must be filled.',
    'required_select' => 'Must be selected.',
    'unique_email' => 'Email already registered.',
];
